            ...
            <div class="flex flex-row items-center justify-center gap-6">
                <div class=" cursor-pointer hover:shadow-xl hover:border-opacity-0 transform hover:-translate-y-2 transition-all duration-200 ">
                    <a href="{{url('/news') . '/' . $page->category_id }}"
                        class="p-3 bg-{{$page->css_name}}-light hover:bg-{{$page->css_name}} text-white text-xs md:text-base line-clamp-1" rel="noreferrer">
            ...
            <ul>
                @foreach($linkList as $link)
                    <li class="pl-4 animation-left-to-right">
                        ...
                        @endif
                        <a href="{{$link->link}}" target="blank" rel="noreferrer">{{$link->title}}</a>
                    </li>
                    @if ($loop->last)
                            <div class="p-2"></div>
                    @endif
            ...
            <ul>
                @foreach($fileList as $file)
                    <li class="pl-4 animation-left-to-right">
                        ...
                        @endif
                        <a href="{{url('/download') . '/' . $file->slug}}" target="blank" rel="noreferrer">{{$file->title}}</a>
                    </li>
                    @if ($loop->last)
                            <div class="p-2"></div>
                    @endif
